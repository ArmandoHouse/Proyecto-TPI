        ...
        <div class="col-md-3">
            ...
            <select class="form-select" id="rol" name="rol">
                ...
                <?php foreach ($roles as $rol): ?>
                    <option value="<?= esc($rol) ?>" <?= (isset($rolSeleccionado) && $rolSeleccionado == $rol) ? 'selected' : '' ?>>
                        <?= esc(ucfirst($rol)) ?>
                    </option>
                <?php endforeach; ?>
            </select>
                                    ...
                                    <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton<?= $usuario['id'] ?>">
                                        <?php if ($usuario['id'] != session()->get('usuario_id')): ?>
                                            <?php if ($usuario['estado'] === 'suspendido'): ?>
                                                <li>
                                                    <a class="dropdown-item" href="<?= base_url('admin/usuarios/activar/' . $usuario['id']) ?>">
                                                        <i class="fa-solid fa-user-check me-2"></i>Activar
                                                    </a>
                                                </li>
                                            <?php else: ?>
                                                <li>
                                                    <a class="dropdown-item" href="<?= base_url('admin/usuarios/suspender/' . $usuario['id']) ?>">
                                                        <i class="fa-solid fa-user-slash me-2"></i>Suspender
                                                    </a>
                                                </li>
                                            <?php endif; ?>
                                            <li>
                                                <a class="dropdown-item text-danger" href="<?= base_url('admin/usuarios/eliminar/' . $usuario['id']) ?>" onclick="return confirm('¿Seguro que deseas eliminar este usuario?');">
